...
disk_usage=$(df -h | awk '{print $2}' | head -n3 | awk 'NF {s=$0} END {print s}')#db_connections=$(mysql -uroot -pexxxxxx -s -N -e "show processlist" | wc -l)
db_connections=6
cld_alert() {    
    nwconn=$1    
	cpu_usage=$2    
	disk_usage=$3    
	db_connections=$4    
	message=$5    touch /tmp/alert.txt && > /tmp/alert.txt    
	date=$(date)    
	echo -e "$date\n" > /tmp/alert.txt    
...
message="$message\n\nCPU usage exceeded configured usage limit \nCurrent CPU usage is $cpu_usage\nConfigured CPU usage is $expected_cpuusage\n\n\n\n\n"        
	...
	if [ "$httpdconn" -ge "$expected_httpdconn" ]; 
then        
